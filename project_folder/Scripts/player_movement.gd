...
func _physics_process(delta: float) -> void:
	...
	# বিশেষ স্টেটগুলোর জন্য আলাদা প্রক্রিয়া
	...
	# দেয়াল চেক (ওয়াল স্লাইড) - null থাকলে skip
	if wall_detector:
		_handle_wall_detection()

	# লেজ গ্র্যাব চেক (শুধু শূন্যে পড়ার সময়)
	if not is_on_floor():
		_check_ledge_grab()
...
func _play_footstep() -> void:
	if not footstep_player and not footstep_player_extra:
		return   # অডিও প্লেয়ার ইন্সটল না থাকলে কিছু করবে না
...
func _change_state(new_state: PlayerState) -> void:
	if current_state == new_state:
		return
	# কিছু স্টেট থেকে বের হওয়া আটকানো (ইন্টারাপ্ট রোধ)
